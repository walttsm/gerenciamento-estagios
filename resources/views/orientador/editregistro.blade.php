...
@section('content')
    ...
    <div class="flex justify-center">
        <div class="rounded-lg shadow-lg bg-white text-center max-w-xl">
            ...
            <form action="{{route('registro.editRegistros', $r->id)}}" method="POST" enctype="multipart/form-data" class="p-9">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div class="form-group pb-8 p-3">
                        ...
                        <select name="aluno_id" id="">
                            @foreach($alunos as $a)
                                @if($aluno_select->id == $a->id)
                                    <option value={{$a['id']}} selected>{{$a['nome_aluno']}}</option>
